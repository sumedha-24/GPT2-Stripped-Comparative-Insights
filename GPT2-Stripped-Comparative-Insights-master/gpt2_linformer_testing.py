...
import wandb
import numpy as np
from hellaswag_linformer import render_example, iterate_examples
import tiktoken
...
if master_process:
    ...
    wandb.run.tags = ["GPT2", "124M params", "10B tokens", "Linformer Attention", "Gelu", "testing", "k=216"]
...
for epoch in range(max_steps):
    ...
    if epoch != last_step:
        ...
        if master_process: print(f"Loaded: {save_folder}/model_{epoch}.pth")
    else: 
        raw_model.load_state_dict(torch.load(f"{save_folder}/final_epoch_model.pth", weights_only=True))
        if master_process: print(f"Loaded: {save_folder}/final_epoch_model_.pth, at epoch {epoch}")
    ...
    max_length = 32
    real_tokens = enc.encode("Hello, I'm a language model,")
    real_tokens = torch.tensor(real_tokens, dtype=torch.long)

    tokens = torch.zeros(1024, dtype=torch.long)
    tokens[tokens.size(0) - real_tokens.size(0):] = real_tokens

    tokens = tokens.unsqueeze(0).repeat(num_return_sequences, 1)
    xgen = tokens.to(device)
    current_length = real_tokens.size(0)
    ...
    while current_length < max_length:
        ...
        with torch.no_grad():
            ...
            # xgen[:, current_length] = xcol.squeeze(1)
    ...
    for i in range(num_return_sequences):
        tokens = xgen[i, xgen.size(1) - max_length:].tolist()
        print(tokens)
